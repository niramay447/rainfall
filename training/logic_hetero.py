...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(charge_bar):
        ...
        # Log1p transform: compress skewed rainfall distribution
        # Applied to rainfall feature (idx 0) in x, and to targets y
        # Inverse (expm1) is applied in test_model for interpretable metrics
        y = torch.log1p(y)
        x = x.clone()
        x[:, 0] = torch.log1p(x[:, 0])
...
def test_model(model, mapping_df, dataloader, device, fold=0, experiment_name= "test"):
    ...
    # === GLOBAL METRICS
    # ============================================================
    # Inverse transform back to mm scale for interpretable metrics and plots
